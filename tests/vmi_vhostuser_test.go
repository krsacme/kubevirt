...
var _ = Describe("Vhostuser", func() {
	...
	Describe("[rfe_id:][crit:][vendor:cnv-qe@redhat.com][level:component]VirtualMachineInstance using different types of interfaces.", func() {
		Context("VirtualMachineInstance with vhostuser interface", func() {
			It("[test_id:]should create a virtual machine with one interface", func() {
				ifaceList := []v1.Interface{
					{Name: "default", InterfaceBindingMethod: v1.InterfaceBindingMethod{
						Masquerade: &v1.InterfaceMasquerade{}}},
					{Name: "vhostnet1", InterfaceBindingMethod: v1.InterfaceBindingMethod{
				...
				disk1 := tests.ContainerDiskFor(tests.ContainerDiskCirros)
				vmi1 := tests.NewRandomVMIWithEphemeralDisk(disk1)
				vmi1.Spec.Domain.Devices.Interfaces = ifaceList
				vmi1.Spec.Networks = netList
				vmi1.Spec.Domain.Resources.Requests[kubev1.ResourceMemory] = resource.MustParse("1Gi")
				vmi1.Spec.Domain.Memory = memory
				_, err = virtClient.VirtualMachineInstance(tests.NamespaceTestDefault).Create(vmi1)
				Expect(err).ToNot(HaveOccurred())
				tests.WaitUntilVMIReady(vmi1, tests.LoggedInCirrosExpecter)

				//disk2 := tests.ContainerDiskFor(tests.ContainerDiskCirros)
				//vmi2 := tests.NewRandomVMIWithEphemeralDisk(disk2)
				//vmi2.Spec.Domain.Devices.Interfaces = ifaceList
				//vmi2.Spec.Networks = netList
				//vmi2.Spec.Domain.Resources.Requests[kubev1.ResourceMemory] = resource.MustParse("1Gi")
				//vmi2.Spec.Domain.Memory = memory
				//_, err = virtClient.VirtualMachineInstance(tests.NamespaceTestDefault).Create(vmi2)
				//Expect(err).ToNot(HaveOccurred())
				//tests.WaitUntilVMIReady(vmi2, tests.LoggedInCirrosExpecter)
